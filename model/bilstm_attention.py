...
────────────
    ...
    LayerNorm(F)              stabilise inputs across feature dim
        │
    BiLSTM ×3 stacked         each layer: hidden=128, bidirectional → out=256
        │
...
Why BiLSTM over LSTM?
    A forward LSTM only sees past frames; a BiLSTM additionally sees future
...
class BadmintonBiLSTM(nn.Module):
    """
    ...
    num_layers  : int  — stacked BiLSTM layers (default 3)
    ...
    def __init__(
        ...
        bilstm_out_dim = hidden_size * 2   # 256 with default settings

        # ── Attention ─────────────────────────────────────────────────────
        self.attention = AttentionLayer(bilstm_out_dim)

        # ── Classifier head ───────────────────────────────────────────────
        self.classifier = nn.Sequential(
            nn.Linear(bilstm_out_dim, fc_hidden),
            nn.GELU(),
            nn.Dropout(dropout),
